...
async function checkAdminAccess() {
  ...
  adminSessionUser = userData.user;
  return adminSessionUser;
}
...
function isOfflineProductCategory(category) {
  return String(category || "").trim().toLowerCase() === "offline";
}
...
function renderOfflineStockEditor(product) {
  ...
  form.elements.stock.title = isOffline ? "สต็อกหมวด Steam Offline จะนับจากรายการที่พร้อมขายด้านล่าง" : "";
  ...
  if (product?.id && cached === null) {
    textarea.value = "";
    textarea.placeholder = "กำลังโหลดสต็อก Steam Offline...";
      ...
      loadOfflineStockItems(product.id)
        .then((items) => {
          if (selectedProduct()?.id === product.id) {
            ...
            textarea.placeholder = "email@example.com | password | note";
...
function validateOfflineStockLines(lines) {
  lines.forEach((line, index) => {
    if (line.length > 2000) {
      throw new Error(`สต็อก Steam Offline บรรทัดที่ ${index + 1} ยาวเกินไป`);
    }
  });
}
...
function openOfflineStockManager(productId) {
  ...
  setStatus("เปิดช่องจัดการสต็อก Steam Offline แล้ว");
}
...
function adminProductErrorMessage(error) {
  ...
  if (
    ...
    return "ยังไม่ได้รัน supabase-offline-stock-items.sql หรือสิทธิ์ offline stock ยังไม่พร้อม";
  }
...
function adminStockErrorMessage(error) {
  ...
  if (message.includes("OFFLINE_STOCK_REQUIRES_ITEMS")) {
    return "สินค้า Steam Offline ต้องจัดการสต็อกแบบ 1 บรรทัดต่อ 1 ชิ้นในฟอร์มสินค้า";
  }
            ...
            isOffline
              ? `
                <div class="offline-stock-card-note">
                  สต็อกหมวด Steam Offline ต้องจัดการเป็นรายการ 1 บรรทัดต่อ 1 ชิ้น
                </div>
...
async function changeStock(productId, nextStock, action) {
  ...
  if (isOfflineProductCategory(product.category)) {
    ...
    switchPanel("products");
    renderProductForm();
    setStatus("สินค้า Steam Offline ต้องเพิ่มสต็อกจากรายการ 1 บรรทัดต่อ 1 ชิ้น");
    showAdminToast("กรุณาจัดการสต็อก Steam Offline ในฟอร์มสินค้า", "info");
    return;
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (admin) {
      ...
      showAdminLogin("กรุณาเข้าสู่ระบบด้วยอีเมลแอดมิน");
    }
  } catch (error) {
    showAdminLogin(adminAuthErrorMessage(error));
  }
